...
export async function POST(request) {
  try {
    ...
    const totalUpdated = results.alcohol.updated + results.naciagi.updated + results.suchy.updated;
    const hasErrors = results.alcohol.error || results.naciagi.error || results.suchy.error;
...
export async function GET() {
  return Response.json({
    ...
    usage: "POST with { confirm: 'ZERO_ALL_QUANTITIES' } to reset all product quantities",
    options: {
      category: "Optional: 'alcohol', 'naciagi', or 'suchy' to reset specific category"
    }
  });
}