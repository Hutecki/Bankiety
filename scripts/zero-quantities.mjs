...
async function zeroAlcoholQuantities() {
  try {
    ...
    if (isDryRun) {
      logger.info("[DRY RUN] Would reset aktualnaIlosc to 0 for all alcohol products");
...
async function zeroNaciagiQuantities() {
  try {
    ...
    if (isDryRun) {
      logger.info("[DRY RUN] Would reset aktualnaIlosc to 0 for all naciagi products");
      return { updated: totalNaciagi, errors: 0 };
    }
    
    // Reset quantities
    const result = await Naciagi.updateMany(
      {},
      { $set: { aktualnaIlosc: 0 } }
    );
...
async function zeroSuchyQuantities() {
  try {
    ...
    if (isDryRun) {
      logger.info("[DRY RUN] Would reset aktualnaIlosc to 0 for all suchy products");
      return { updated: totalSuchy, errors: 0 };
    }
    
    // Reset quantities
    const result = await Suchy.updateMany(
      {},
      { $set: { aktualnaIlosc: 0 } }
    );
